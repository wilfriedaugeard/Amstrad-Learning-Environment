...
SDLKey code, Uint8 type, SDLMod mod = KMOD_NONE
*/

SDL_Event uintToSDLEvent(unsigned int toConvert) {
      SDL_Event translated;
      SDLKey key;
      SDLMod mod;

      unsigned int shiftMod;
      unsigned int getMod = toConvert >> 16;

      if(getMod == KMOD_CTRL) {
            shiftMod = KMOD_CTRL << 16;
            key = (SDLKey)(toConvert ^ shiftMod);
            mod = KMOD_LCTRL;
      }

      else if(getMod == KMOD_SHIFT) {
            shiftMod = KMOD_SHIFT << 16;
            key = (SDLKey)(toConvert ^ shiftMod);
            mod = KMOD_LSHIFT;
      }

      else if(getMod == KMOD_ALT) {
            shiftMod = KMOD_ALT << 16;
            key = (SDLKey)(toConvert ^ shiftMod);
            mod = KMOD_LALT;
      }

      else if(getMod == KMOD_META) {
            shiftMod = KMOD_META << 16;
            key = (SDLKey)(toConvert ^ shiftMod);
            mod = KMOD_LMETA;
      }

      else {
            key = (SDLKey)(toConvert);
            mod = KMOD_NONE;
      }

      translated.key.keysym.sym = key;
      translated.key.keysym.mod = mod;
      return translated;
}

CPCScreen getScreen() {
      // back_surface;
      int width = back_surface->w;
      int height = back_surface->h;

      CPCScreen pixels;
      pixels.width = width;
      pixels.height = height;

      // CRGBColor pixelFetcher;
      for(int line = 0; line < height; line++) {
            for(int column = 0; column < width; column++) {
                  unsigned int * pixel = (unsigned int *)back_surface->pixels + column + line * width;
